...
public class Routes {
    ...
    public Router createRoutes(MovieRepository movieRepository) {
        ...
        router.route(ALL_MOVIES_ENDPOINTS).handler(
                CorsHandler.create().allowedHeaders(Headers.allowedHeaders).allowedMethods(HttpMethods.allowedMethods));
    ...
    private void routeLogin(Router router, JWTAuth authProvider) {
        router.route("/login").handler(routingContext -> {
            this.configRetriever.getConfig().onSuccess(configFile -> {
    ...
    private boolean validateAuth(JsonObject configFile, JsonObject requestBody) {
        String incomingUser = requestBody.getString(USER);
        String incomingPassword = requestBody.getString(USER_PASSWORD);

        if (incomingUser == null || incomingPassword == null) {
            return false;
        }

        boolean userMatch = MessageDigest.isEqual(
                configFile.getString(USER).getBytes(),
                incomingUser.getBytes()
        );

        return userMatch && Password.verify(incomingPassword, configFile.getString(USER_PASSWORD));
    }
